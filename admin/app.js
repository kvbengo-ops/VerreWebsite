...
async function signOut(){
  localStorage.setItem('verre.pos.locked','1');
...
function drawOrders(){
  const query=$('#order-search').value.toLowerCase(),status=$('#order-status').value;
  const rows=state.orders.filter(order=>(!query||(order.ref+' '+order.customer_name+' '+order.customer_email).toLowerCase().includes(query))&&(!status||order.status===status));
  $('#orders-table').innerHTML=rows.length?`<div class="table-wrap"><table><thead><tr><th>Ref</th><th>Customer</th><th>Total</th><th>Channel</th><th>Status</th><th>Created</th></tr></thead><tbody>${rows.map(order=>`<tr class="${order.is_oversell?'low':''}"><td><button class="order-link" type="button" data-order="${order.id}"><strong>${esc(order.ref)}</strong>${order.is_oversell?'<br><small>Needs attention — sold twice</small>':''}</button></td><td>${esc(order.customer_name||'Walk-in')}<br><small>${esc(order.customer_email||'')}</small></td><td>${money(order.total_cents)}</td><td>${order.channel}</td><td><span class="status ${order.status}">${order.status}</span></td><td>${date(order.created_at)}</td></tr>`).join('')}</tbody></table></div>`:(state.orders.length?emptyState('✦','Nothing matches','No order matches that search or status filter.'):emptyState('✦','No orders yet','Web inquiries and POS sales will both land here.'));
  $$('[data-order]').forEach(button=>button.onclick=()=>orderDetail(button.dataset.order));
}
...
async function sessions(){
  ...
  setBody(`<div class="toolbar"><button id="open-session" class="primary">Start session</button></div><div class="table-wrap"><table><thead><tr><th>Label</th><th>Operator / device</th><th>Opened</th><th>Opening float</th><th>Expected cash</th><th>Counted cash</th><th>Variance</th><th>Status</th><th></th></tr></thead><tbody>${state.sessions.map(s=>`<tr><td>${esc(s.label)}</td><td>${esc(s.operator_email||'')}<br><small>${esc(s.device_label||'')}</small></td><td>${date(s.opened_at)}</td><td>${money(s.opening_float_cents)}</td><td>${money(s.expected_cash_cents)}</td><td>${s.closing_cash_cents==null?'—':money(s.closing_cash_cents)}</td><td>${s.variance_cents==null?'—':money(s.variance_cents)}</td><td><span class="status ${s.closed_at?'fulfilled':'inquiry'}">${s.closed_at?'closed':'open'}</span></td><td>${s.closed_at?'':`<button data-close-session="${s.id}">Close</button>`}</td></tr>`).join('')}</tbody></table></div>`);
